...
public class VendorController {
    ...
    public ResponseEntity<ApiResponse<Book>> updateBookImage(
            @PathVariable Long bookId,
            @RequestParam String imageUrl,
            Authentication authentication) {

        logger.info("PUT /api/vendor/books/{}/image - Updating book image", bookId);

        try {
            Long userId = getUserIdFromAuth(authentication);
            Book book = vendorService.updateBookImage(userId, bookId, imageUrl);

            logger.info("Book image updated successfully - Book ID: {}", bookId);
            return ResponseEntity.ok(ApiResponse.success("Book image updated", book));

        } catch (Exception e) {
            logger.error("Error updating book image - Book ID: {} - Error: {}", bookId, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }

    @GetMapping("/orders")
    public ResponseEntity<ApiResponse<List<Order>>> getVendorOrders(Authentication authentication) {
        logger.info("GET /api/vendor/orders - Fetching vendor orders");

        try {
            Long userId = getUserIdFromAuth(authentication);
            List<Order> orders = vendorService.getVendorOrders(userId);

            logger.info("Retrieved {} orders for vendor", orders.size());
            return ResponseEntity.ok(ApiResponse.success("Vendor orders retrieved", orders));

        } catch (Exception e) {
            logger.error("Error fetching vendor orders - Error: {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Order>> updateOrderStatus(
        ...
        logger.info("PUT /api/vendor/orders/{}/status - Updating order status to: {}", orderId, status);

        try {
            Long userId = getUserIdFromAuth(authentication);
            Order order = vendorService.updateOrderStatus(userId, orderId, status);

            logger.info("Order status updated by vendor - Order ID: {}, Status: {}", orderId, status);
            return ResponseEntity.ok(ApiResponse.success("Order status updated", order));

        } catch (Exception e) {
            logger.error("Error updating order status - Order ID: {} - Error: {}", orderId, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
}